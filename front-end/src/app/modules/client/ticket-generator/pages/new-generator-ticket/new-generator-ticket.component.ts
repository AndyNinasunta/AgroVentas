...
export class NewGeneratorTicketComponent implements OnInit {
  ...
  generateTicket() {
    ...
    const userAux: UserI = JSON.parse(JSON.stringify(this.userForm.getRawValue()));
    ...
    this._ticketService.generateTicket(userAux).pipe(
  ...
  generatedTicket(queue: number, user: UserI) {
    ...
    let dateNow:string=formatoMap.dd+'/'+formatoMap.mm+'/'+formatoMap.yyyy;
