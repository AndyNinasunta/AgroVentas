        ...
        if (cIdentification.length !== 10) {
            ...
            if (thirdDigit < 6) {
                ...
                for (let i = 0; i < (cIdentification.length - 1); i++) {
                    digit = parseInt(cIdentification.substring(i, i + 1)) * coefValCI[i];
    ...
    getErrorValidateMessage(_form: FormGroup, p_control: any) {
        ...
        if (_form.get(p_control).hasError('identification')) {
            ...
            switch (_form.get(p_control).getError('identification')) {
                case 2:
                    message = 'El número de cédula o RUC ingresado es incorrecto.';
                    break;
                case 3:
                    message = 'El número de RUC ingresado es incorrecto.';
                    break;
                default:
                    message = 'El número de identificación ingresado es incorrecto';
                    break;
            }
            return message;
        }
        ...
        message =
            _form.get(p_control).hasError('required') ? 'El campo es requerido' :
                _form.get(p_control).hasError('email') ? 'No pertenece a un correo valido' :
                    _form.get(p_control).hasError('maxlength') ?
                        `La longitud maxima es: ${_form.get(p_control).getError('maxlength')
                            .requiredLength}` :
